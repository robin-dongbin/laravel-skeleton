...
export interface UseQueryBuilderReturn<T extends Record<string, any>>
	extends UseFormReturnType<InitialValues<T>> {}
...
const QueryBuilderContext = createContext<QueryBuilderContextValue<any> | null>(
	null,
)
...
export const QueryBuilderProvider = <T,>({
	...
	useEffect(() => {
		const values = mapValues(
			initialValues,
			(value, key) => searchParams.get(String(key)) ?? value,
		)
